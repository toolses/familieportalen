    ...
    @if (data.children().length === 0) {
      ...
      <div class="px-4 pt-2 pb-6 space-y-4" appSwipe (swipeLeft)="nextDay()" (swipeRight)="prevDay()">
        ...
        @if (calendarNeedsReconnect()) {
          <button (click)="reconnectCalendar()"
                  class="w-full flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-2xl px-4 py-3 text-left active:scale-[0.98] transition-all">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm font-semibold text-blue-800">Koble til kalender igjen</p>
              <p class="text-xs text-blue-500">Google-tilkoblingen ble avbrutt – trykk for å gjenopprette</p>
            </div>
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-blue-400 shrink-0"><polyline points="9 18 15 12 9 6"/></svg>
          </button>
        }
                ...
                @for (event of todayGoogleEvents(); track event.id) {
                  <div class="flex gap-3 items-start bg-gray-50 rounded-xl p-3">
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      @if (event.startTime) {
                        <p class="text-xs text-gray-400 mt-0.5">{{ formatTime(event.startTime) }}@if (event.endTime) { – {{ formatTime(event.endTime) }}}</p>
                      } @else {
...
export class DashboardComponent {
  ...
  readonly calendarNeedsReconnect = computed(() => this.google.needsReconnect());

  async reconnectCalendar(): Promise<void> {
    await this.google.reconnectCalendar();
  }
